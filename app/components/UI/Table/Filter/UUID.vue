<script setup lang="ts">
const props = defineProps<{ field: string }>()
defineEmits<{ confirm: [string | null, Event], cancel: [Event] }>()
const expr = ref<string | null>(null)

const objs = defineModel<{ operator: string, value: string | undefined }[]>({ default: [{ operator: 'eq', value: undefined }] })

function isBinary(op: string) {
  return ['eq', 'ne'].includes(op)
}

watchEffect(() => {
  const result: string[] = []
  if (objs.value.length) {
    for (let index = 0; index < objs.value.length; index++) {
      const obj = objs.value[index]
      if (obj) {
        if (isBinary(obj.operator)) {
          // 处理空字符串
          if (!obj.value)
            obj.value = undefined
          else
            result.push(`${props.field} ${obj.operator} ${obj.value}`)
        }
        else if (['is_null', 'is_not_null'].includes(obj.operator)) {
          result.push(`${props.field} ${obj.operator}`)
        }
      }
    }
  }
  if (result.length) {
    expr.value = result.join(' and ')
  }
  else {
    expr.value = null
  }
})
const invalid = ref(false)
</script>

<template>
  <UITableFilterLayout
    @cancel="(e) => {
      objs = [{ operator: 'eq', value: undefined }]
      $emit('cancel', e)
    }"
    @confirm="e => !invalid && $emit('confirm', expr, e)"
  >
    <template
      v-for="obj, index in objs" :key="index"
    >
      <a-select
        v-model="obj.operator"
        size="mini"
        class="mb-2"
        :options="[
          { label: $t('common.filterExpr.eq'), value: 'eq' },
          { label: $t('common.filterExpr.ne'), value: 'ne' },
          { label: $t('common.filterExpr.is_null'), value: 'is_null' },
          { label: $t('common.filterExpr.is_not_null'), value: 'is_not_null' },
        ]"
      />
      <a-input
        v-if="isBinary(obj.operator)"
        v-model="obj.value"
        size="mini"
        :max-length="37"
        @blur="() => {
          invalid = obj.value !== undefined && !/^[0-9a-fA-F]{8}\b-[0-9a-fA-F]{4}\b-[0-9a-fA-F]{4}\b-[0-9a-fA-F]{4}\b-[0-9a-fA-F]{12}$/.test(obj.value)
        }"
      >
        <template #prepend>
          <icon-search />
        </template>
      </a-input>

      <AAlert v-if="invalid" type="error" class="mt-4 !text-xs">
        {{ $t('validation.invalidUUID') }}
      </AAlert>
    </template>
  </UITableFilterLayout>
</template>
